...
イントロとエンディングは、それぞれ設定画面で次の項目を調整できます。

- 表示時間: 1～60秒（画像・動画共通。短い動画は自動ループ）
- ズーム: `Static` / `Slow Zoom In` / `Slow Zoom Out`
- 音声: `Original` / `BGM` / `Original + BGM` / `Mute`
- BGM音量: 0～100%

`BGM` を選ぶ場合は、通常のBGMと同じく `assets/bgm` の先頭の `mp3` または `wav` が使われます。素材音声がない画像でもBGMを付けられます。
...
Light EffectはMotion・Overlayの後、字幕・タイトルの前に適用されます。Light EffectでFFmpegエラーになる場合は、最新版であることを確認してから再生成してください。
...
1. 「Analytics」タブを開きます。
2. 「CSVを読み込む」を押し、YouTube Studio または TikTok Studio のCSVを選択します。複数CSVをまとめて選択できます。
3. フォルダ内のCSVをまとめて取り込む場合は「CSVフォルダを読み込む」を押します。
4. 集計、ランキング、ジャンル分析、タイトル分析、動画一覧、Project Analytics、分析コメントを確認します。
5. 必要に応じてタイトル、ジャンル、再生数以上、投稿日、評価で検索します。
6. 「分析結果CSV保存」で整形済みの分析結果をCSVとして保存できます。
...
Phase7.4では、プロジェクト数が増えても整理しやすいように、`ジャンル > カテゴリ > シリーズ` の階層を追加しました。

使い分け:

- ジャンル: 大分類です。例: 宇宙、株、AIニュース
- カテゴリ: ジャンル内の小分類です。例: 宇宙 > ブラックホール、宇宙 > 恒星
- シリーズ: 連続投稿のまとまりです。例: ブラックホール基礎 #003

カテゴリ管理:

1. 左側の「一括ネタ管理」内にある「カテゴリ管理」を開きます。
2. 管理するジャンルを選びます。
3. 「追加」「編集」「削除」でカテゴリを管理します。
4. 「上へ」「下へ」で並び替えできます。

カテゴリ削除時、そのカテゴリを使用中のプロジェクトがある場合は警告します。既存プロジェクトのカテゴリは勝手に削除・変更しません。

プロジェクトへのカテゴリ設定:

- 制作設定でジャンルを選ぶと、そのジャンルのカテゴリ候補だけが表示されます。
- カテゴリ欄は自由入力できます。
- 自由入力したカテゴリは確認後にカテゴリ一覧へ追加できます。
- 一括ネタ管理の「一括作成カテゴリ」「一括作成シリーズ」から、複数プロジェクトへ同じカテゴリを反映できます。

絞り込み:

- 左側のプロジェクト一覧で、ジャンル、カテゴリ、シリーズ、タグ、投稿状態、制作状態、評価、再生数を組み合わせて検索できます。
- 「未分類」を選ぶと、カテゴリ未設定の既存プロジェクトだけを表示できます。
- 「絞り込み解除」で条件を1クリックで初期化できます。
- 表示件数は `表示中: 24件 / 全186件` の形式で確認できます。

一括カテゴリ変更:

1. プロジェクト一覧で複数プロジェクトを選択します。
2. カテゴリ管理で反映したいジャンル・カテゴリを選びます。
3. 「選択プロジェクトへ一括反映」を押します。
4. 必要に応じてシリーズ名、追加タグ、削除タグを入力します。

カテゴリ別Analytics:

- Analyticsの「ジャンル分析」にカテゴリ別集計が表示されます。
- カテゴリ別の動画本数、総再生数、平均再生数、平均いいね率、平均視聴率、5段階評価を確認できます。
- AIアドバイザーのおすすめテーマにも、好調カテゴリが反映されます。

## Project Analytics / 個別動画分析

Phase7.2〜Phase7.3では、Analyticsで読み込んだCSVデータを `projects` 内の各動画プロジェクトへ自動で紐付け、個別成績と改善案を確認できます。

YouTube Studio CSV:

- `表データ.csv`: 動画ごとの成績として利用します。
- `グラフデータ.csv`: 日別推移グラフとして利用します。
- `合計.csv`: チャンネル全体の合計値として利用します。

ファイル名だけではなく列名から種類を判定するため、英語名やリネーム済みCSVでも利用できる範囲で読み込みます。一部ファイルしかない場合もエラーにはせず、取得できた列だけを使います。

TikTok Studio CSV:

- タイトル、視聴回数、いいね、コメント、シェア、保存、平均視聴時間、完視聴率、フォロワー増減など、CSVに存在する列だけを利用します。

プロジェクト自動紐付け:

- `title.txt`
- `topic.txt`
- `project.json` のタイトル / テーマ
- 動画ファイル名
- プロジェクトフォルダ名

上記とCSVタイトルを比較し、完全一致、正規化一致、部分一致、類似度一致の順に紐付けます。正規化では、空白、全角半角、大小文字、記号、ハッシュタグ、絵文字、`#Shorts`、末尾番号などを考慮します。

手動紐付け:

1. AnalyticsタブでCSVを読み込みます。
2. 「未紐付け管理」タブを開きます。
3. 未紐付け動画を選び、紐付け先プロジェクトを選択します。
4. 「紐付け保存」を押します。

手動紐付けは `analytics_links.json` に保存され、次回CSV読み込み後も維持されます。CSVと紐付いたプロジェクトには `project.json` の `analytics` と `posting_status` が保存されます。既存の `posted.txt` は削除・上書きしません。

個別動画分析:

- 制作ウィザードの「個別分析」タブで、現在のプロジェクトのYouTube / TikTok成績を確認できます。
- URLがCSVに含まれる場合、「YouTube URLを開く」「TikTok URLを開く」ボタンからブラウザで開けます。
- データがない項目は「未取得」と表示されます。
- 5段階評価、伸びた理由、次回改善案、続編候補、YouTube / TikTok横断比較をルールベースで表示します。
...
改善提案は、再生数、いいね率、コメント率、CTR、平均視聴率、完視聴率、登録者/フォロワー増減など、取得できる指標だけを使います。指標が不足している場合は「分析データ不足」と表示します。

CSV文字コードと形式:

- UTF-8 / UTF-8 BOM / CP932 / Shift-JIS に対応します。
- カンマ区切り、数値内のカンマ、パーセント表記、空欄、日本語/英語列名に対応します。

よくあるエラー:

- `CSVにタイトルまたは再生数の列がありません。`: 表データではないCSV、または列名が未対応です。表データCSVも一緒に選択してください。
- `CSVの文字コードを判定できませんでした。`: CSVをExcelで開き、UTF-8 CSVとして保存し直してください。
- 個別分析が「未取得」のまま: タイトルがプロジェクトと一致していません。「未紐付け管理」から手動で紐付けてください。
...
- 0.7.4 Phase7.4: ジャンル配下のカテゴリ管理、階層フィルター、複合フィルター、未分類整理、一括カテゴリ変更、カテゴリ別Analyticsに対応
- 0.7.3 Phase7.2〜7.3: Project Analytics、CSVとプロジェクト紐付け、個別成績、改善提案、続編候補、YouTube/TikTok横断比較に対応
